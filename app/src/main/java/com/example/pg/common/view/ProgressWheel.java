package com.example.pg.common.view;
...
public class ProgressWheel extends View {
	...
	private void setupBounds(int layout_width, int layout_height) {
		...
		if (!fillRadius) {
			// Width should equal to Height, find the min values to setup the
			// circle
	...
	/**
	 * Set the progress to a specific values, the bar will be set instantly to
	 * that values
	 * 
	 * @param progress
	...
	/**
	 * Set the progress to a specific values, the bar will smoothly animate until
	 * that values
	 * 
	 * @param progress
	 *            the progress between 0 and 1
	 ...
	 *         equals on full turn in one second), this values also is applied
	 ...
	 * on full turn in one second), this values also is applied for the
	...
	public interface ProgressCallback {
		/**
		 * Method to call when the progress reaches a values in order to avoid
		 ...
		 * completes an animation cycle, with, the progress values is -1.0f
		 * 
		 * @param progress
		 *            a double values between 0.00 and 1.00 both included
		 */
